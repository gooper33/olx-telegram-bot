...
# ===== ОТПРАВКА ЧЕРЕЗ API (ТОЛЬКО КУКИ) =====
def send_message_via_api(ad_url, message_text):
    """Отправляет сообщение в чат через API используя только куки"""
    ...
    try:
        ...
        # Пробуем создать чат и отправить сообщение
        ...
        if response.status_code == 401:
            return {"error": "❌ Сессия устарела. Обновите куки!"}
        
        if response.status_code == 403:
            return {"error": "❌ Доступ запрещен. Проверьте куки!"}
        
        if response.status_code == 404:
            return {"error": "❌ Объявление не найдено (возможно, снято с публикации)"}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 OLX Sender\n\n"
        "📌 Бот отправляет сообщения в чаты OLX!\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "help":
        await query.edit_message_text(
            ...
            "Напишите текст сообщения в чат\n"
            "Затем нажмите «Отправить сообщение»",
            parse_mode='HTML',
    ...
    if data == "send_message":
        ...
        # Проверяем, есть ли текст в user_data
        message_text = context.user_data.get('message_text')
        if not message_text:
            await query.edit_message_text(
                "📝 Сначала напишите текст сообщения в чат!\n\n"
                "Например: Bună ziua! Sunt interesat de produs.",
                reply_markup=back_button()
            )
            return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not action:
        # Сохраняем текст как сообщение для отправки
        context.user_data['message_text'] = text
        await update.message.reply_text(
            f"✅ Текст сохранён!\n\n"
            f"📝 {text[:200]}{'...' if len(text) > 200 else ''}\n\n"
            f"Теперь нажмите «📝 ОТПРАВИТЬ СООБЩЕНИЕ» в меню.",
            parse_mode='HTML',
            reply_markup=main_menu()
        )
        return
