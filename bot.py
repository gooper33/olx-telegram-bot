...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cookies_menu":
        d = get_data()
        status = "✅ Загружены" if d['cookies'] else "⬜️ Не загружены"
        keyboard = [
        ...
        await query.edit_message_text(
            ...
            f"Куки нужны для авторизации на OLX.\n"
            f"Отправьте файл или текст с JSON-массивом.",
            parse_mode='HTML',
    ...
    if data == "upload_cookies_text":
        await query.edit_message_text(
            ...
            "Скопируйте JSON из расширения и отправьте.\n\n"
    ...
    if data == "upload_proxies_file":
        await query.edit_message_text(
            "📤 <b>Отправьте файл с прокси</b>\n\n"
            "Файл должен называться <b>proxies.txt</b>",
            parse_mode='HTML',
            reply_markup=back_button()
        )
    ...
    if data == "settings_menu":
        ...
        await query.edit_message_text(
            ...
            "Задержка — время между отправками.",
            parse_mode='HTML',
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    ...
    if data == "start_send":
        ...
        errors = []
        if not d['cookies']:
            errors.append("⬜️ Нет куки (загрузите авторизацию)")
        if not d['texts']:
            errors.append("⬜️ Нет текстов для рассылки")
        if not d['ads']:
            errors.append("⬜️ Нет объявлений (загрузите CSV)")
        ...
        msg = (
            ...
            f"⏱ Задержка: {get_delay()} сек\n\n"
            f"⚠️ Бот отправит сообщения от вашего имени.\n"
            f"Убедитесь, что всё настроено правильно."
        )
...
# ===== РАССЫЛКА С КРАСИВЫМ ПРОГРЕССОМ =====
async def send_all_messages(context, chat_id, proxies, texts, ads) -> str:
    ...
    for i, ad in enumerate(ads, 1):
        try:
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"⚠️ <b>{i}/{total}</b> {ad.get('title', '')[:40]}\n└ {str(e)[:150]}"
            )
