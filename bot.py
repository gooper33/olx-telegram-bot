...
# Пытаемся импортировать Playwright
try:
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {"delay": 45, "proxy_timeout": 10}
...
def get_proxy_timeout():
    return config.get('proxy_timeout', 10)

def set_proxy_timeout(value):
    config['proxy_timeout'] = value
    save_config(config)
...
async def send_message_via_browser(ad_url, message_text, proxy=None, attempt=0):
    """Отправляет сообщение через браузер с повторными попытками"""
    ...
    if proxy and not check_proxy(proxy):
        return {"error": "❌ Прокси не работает"}
    ...
    try:
        async with async_playwright() as p:
            ...
            try:
                ...
                if attempt < 2 and ("Timeout" in error_msg or "closed" in error_msg):
                    await asyncio.sleep(5)
                    return await send_message_via_browser(ad_url, message_text, proxy, attempt + 1)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 <b>OLX Sender Pro</b>\n\n"
        "📌 <b>Бот для рассылки на OLX.ro</b>\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "stats":
        ...
        msg = (
            ...
            f"🌐 Прокси: {'✅' if d['proxies'] else '⬜️'} ({len(proxies)} шт., рабочих: {len(working)})\n"
            ...
            f"│ {bar} │\n"
            f"└{'─' * 12}┘\n\n"
            f"{'✅ ГОТОВ' if ready else '⬜️ ЗАГРУЗИТЕ ДАННЫЕ'}"
        )
    ...
    if data == "help":
        await query.edit_message_text(
            ...
            "2️⃣ Загрузите прокси (файл или текст)\n"
            "3️⃣ Загрузите CSV с объявлениями\n"
            "4️⃣ Добавьте тексты для рассылки\n"
            "5️⃣ Нажмите «Запустить рассылку»\n\n"
            "📌 <b>Куки:</b> EditThisCookie → Export → отправьте JSON\n"
            "📌 <b>Прокси:</b> http://user:pass@ip:port (каждый с новой строки)\n"
            "📌 <b>CSV:</b> country,title,price,publication,seller,registration,phone,ad_url,image_url,city,category,description",
            parse_mode='HTML',
    ...
    if data == "proxies_menu":
        keyboard = [
            ...
            [InlineKeyboardButton("📤 Загрузить файл", callback_data="upload_proxies_file")],
        ...
        await query.edit_message_text(
            ...
            f"Всего: {len(proxies)} шт.\n"
            f"Рабочих: {len(working)} шт.",
            parse_mode='HTML',
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    ...
    if data == "upload_proxies_file":
        await query.edit_message_text("📤 Отправьте файл proxies.txt", reply_markup=back_button())
    ...
    if data == "list_proxies":
        ...
        for i, p in enumerate(d['proxies']):
            clean = re.sub(r':[^:@]+@', ':****@', p)
            working = "✅" if check_proxy(p) else "❌"
            msg += f"{i+1}. {working} <code>{clean}</code>\n"
        await query.edit_message_text(msg, parse_mode='HTML', reply_markup=back_button())
        return
    ...
    if data == "check_proxies":
        proxies = load_proxies()
        if not proxies:
            await query.edit_message_text("❌ Нет прокси для проверки.", reply_markup=back_button())
            return
        await query.edit_message_text("🔍 <b>ПРОВЕРКА ПРОКСИ...</b>", parse_mode='HTML')
        working = get_working_proxies(proxies)
        await query.edit_message_text(
            f"✅ <b>ПРОВЕРКА ЗАВЕРШЕНА</b>\n\n"
            f"Всего: {len(proxies)} шт.\n"
            f"Рабочих: {len(working)} шт.\n"
            f"Не рабочих: {len(proxies) - len(working)} шт.",
            parse_mode='HTML',
            reply_markup=back_button()
        )
        return
    ...
    if data == "settings_menu":
        keyboard = [
            ...
            [InlineKeyboardButton(f"⏱ Таймаут прокси: {get_proxy_timeout()} сек", callback_data="proxy_timeout_menu")],
            [InlineKeyboardButton("🗑️ ОЧИСТИТЬ ВСЁ", callback_data="clear_all")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="main_menu")]
        ]
        await query.edit_message_text("⚙️ <b>НАСТРОЙКИ</b>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data == "delay_menu":
        ...
        await query.edit_message_text(
            f"⏱ <b>ВЫБЕРИТЕ ЗАДЕРЖКУ</b>\n\nТекущая: <b>{get_delay()} сек</b>",
            parse_mode='HTML',
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return

    if data == "proxy_timeout_menu":
        keyboard = [
            [InlineKeyboardButton("5 сек", callback_data="ptimeout_5")],
            [InlineKeyboardButton("10 сек", callback_data="ptimeout_10")],
            [InlineKeyboardButton("15 сек", callback_data="ptimeout_15")],
            [InlineKeyboardButton("20 сек", callback_data="ptimeout_20")],
            [InlineKeyboardButton("30 сек", callback_data="ptimeout_30")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="settings_menu")]
        ]
        await query.edit_message_text(
            f"⏱ <b>ТАЙМАУТ ПРОКСИ</b>\n\nТекущий: <b>{get_proxy_timeout()} сек</b>",
            parse_mode='HTML',
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return

    if data.startswith("ptimeout_"):
        seconds = int(data.split("_")[1])
        set_proxy_timeout(seconds)
        await query.edit_message_text(f"✅ Таймаут: <b>{seconds} сек</b>", parse_mode='HTML', reply_markup=main_menu())
        return

    if data.startswith("delay_"):
        seconds = int(data.split("_")[1])
        set_delay(seconds)
        await query.edit_message_text(f"✅ Задержка: <b>{seconds} сек</b>", parse_mode='HTML', reply_markup=main_menu())
        return
    ...
    if data == "start_send":
        ...
        if proxies:
            working = get_working_proxies(proxies)
            if not working:
                errors.append("⚠️ Все прокси не работают!")
        ...
        if not PLAYWRIGHT_AVAILABLE:
            await query.edit_message_text(
                "⚠️ <b>PLAYWRIGHT НЕ УСТАНОВЛЕН</b>\n\n"
                "Добавьте playwright в requirements.txt\n"
        ...
        msg = (
            ...
            f"✅ Рабочих прокси: {len(get_working_proxies(proxies))}\n"
            f"⏱ Задержка: {get_delay()} сек\n\n"
            f"⚠️ Бот будет работать как человек\n"
            f"⏳ Примерно {len(d['ads']) * 45 // 60} минут"
        )
    ...
    if data == "confirm_send":
        ...
        proxies = load_proxies()
        working = get_working_proxies(proxies)
        result = await send_all_messages(context, query.from_user.id, working, d['texts'], d['ads'])
...
async def send_all_messages(context, chat_id, proxies, texts, ads) -> str:
    ...
    for i, ad in enumerate(ads, 1):
        try:
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"⚠️ <b>{i}/{total}</b> {str(e)[:150]}"
            )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'add_proxy':
        ...
        working = get_working_proxies(proxies)
        await update.message.reply_text(
            f"✅ Прокси добавлен!\n\nВсего: {len(proxies)} шт.\nРабочих: {len(working)} шт.",
            reply_markup=main_menu()
        )
        user_data['action'] = None
        return
...
def main():
    ...
    if not PLAYWRIGHT_AVAILABLE:
        print("⚠️ ВНИМАНИЕ: Playwright не установлен!")
