...
def parse_csv_text(content: str) -> list:
    ...
    # Находим индексы нужных колонок по вашему формату
    try:
        title_idx = header.index('Название')
        price_idx = header.index('Цена')
        ad_url_idx = header.index('Ссылка на объявление')
    except ValueError:
        # Если не нашли, используем порядок
        title_idx = 0
        price_idx = 1
        ad_url_idx = 6
...
def extract_ad_id_from_url(url):
    ...
    # Ваш формат: https://www.olx.ro/d/oferta/...-IDkJrM8.html
    match = re.search(r'-([A-Za-z0-9]+)\.html$', url)
    if match:
        return match.group(1)
    
    # Альтернативный формат: просто ID в конце
...
def send_offer_via_api(ad_url, offer_price, original_price):
    """Отправляет предложение цены через чат OLX"""
    
    ad_id = extract_ad_id_from_url(ad_url)
    
    if not ad_id:
        return {"error": f"Не удалось извлечь ID из URL: {ad_url}"}
    ...
    message_text = f"Bună ziua! Aș dori să vă fac o ofertă de {offer_price} RON pentru acest produs. Prețul dvs. este {original_price} RON. Vă rog să luați în considerare oferta mea."
    ...
    try:
        ...
        if response.status_code == 404:
            return {"error": f"Объявление {ad_id} не найдено"}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 OLX Price Offer Bot\n\n"
        "💰 Бот отправляет предложения цены!\n\n"
        "1️⃣ Загрузите куки\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "help":
        await query.edit_message_text(
            ...
            "📌 Ваш CSV формат:\n"
            "Название,Цена,Ссылка на объявление\n\n"
            "📌 Как это работает:\n"
            "1. Бот читает цену из CSV\n"
            "2. Предлагает на 1 лей меньше\n"
            "3. Отправляет сообщение в чат продавца",
            parse_mode='HTML',
    ...
    if data == "upload_csv":
        await query.edit_message_text("📤 Отправьте CSV-файл\nФормат: Название,Цена,Ссылка на объявление", reply_markup=back_button())
    ...
    if data.startswith("ad_"):
        ...
        if not original_price:
            await query.edit_message_text(
                ...
                f"Проверьте формат в CSV (например: 750 €, 499 lei)",
                reply_markup=back_button()
            )
            return
