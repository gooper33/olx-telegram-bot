...
def send_message_via_api(access_token, ad_id, message_text, proxy=None):
    """Отправляет сообщение через OLX API v2"""
    headers = {
        "Authorization": f"Bearer {access_token}",
    ...
    try:
        ...
        url = f"https://www.olx.ro/api/v2/ads/{ad_id}/threads"
        ...
        if response.status_code == 201:
            ...
            return {"error": f"Объявление {ad_id} не найдено или недоступно для сообщений"}
        else:
