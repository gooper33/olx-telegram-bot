...
def send_message_via_api(access_token, ad_id, message_text, proxy=None):
    """Отправляет сообщение через OLX API с полным логированием"""
    headers = {
    ...
    try:
        ...
        # ПЕРВЫЙ ВАРИАНТ: отправка в чат объявления
        url = f"https://www.olx.ro/api/partner/ads/{ad_id}/threads"
        payload = {"message": {"text": message_text}}
        
        print(f"📤 Отправка в {url}")
        ...
        # ЕСЛИ ЧАТ УЖЕ СУЩЕСТВУЕТ (409)
        elif response.status_code == 409:
            print("🔁 Чат уже существует, ищем thread_id...")
            ...
            if threads_response.status_code == 200:
                ...
                if thread_data.get('data') and len(thread_data['data']) > 0:
                    ...
                    if thread_id:
                        ...
                        print(f"📥 Статус отправки в чат: {msg_response.status_code}")
                        print(f"📥 Ответ: {msg_response.text[:500]}")
                        
                        if msg_response.status_code == 201:
                            return {"success": True}
                        else:
                            return {"error": f"Ошибка отправки в чат {msg_response.status_code}: {msg_response.text[:200]}"}
                else:
                    return {"error": "Нет активных чатов для этого объявления"}
            else:
                return {"error": f"Ошибка получения чатов {threads_response.status_code}: {threads_response.text[:200]}"}
        
        # ДРУГИЕ ОШИБКИ
        else:
            return {"error": f"Ошибка {response.status_code}: {response.text[:300]}"}
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "stats" or data == "refresh_stats":
        ...
        global tokens, proxies, texts, ads
        tokens = load_tokens()
        proxies = load_proxies()
        texts = load_texts()
        ads = load_ads()
        ...
        msg = (
            ...
            f"👤 Токенов: <b>{len(tokens)}</b>\n"
            f"🌐 Прокси: <b>{len(proxies)}</b>\n"
    ...
    # ===== РАССЫЛКА =====
    if data == "start_send":
        # Обновляем данные перед рассылкой
        global tokens, proxies, texts, ads
        tokens = load_tokens()
        proxies = load_proxies()
        texts = load_texts()
...
async def send_all_messages(context, chat_id) -> str:
    # Загружаем свежие данные перед рассылкой
    current_ads = load_ads()
    current_texts = load_texts()
    current_tokens = load_tokens()
    current_proxies = load_proxies()
    
    total = len(current_ads)
    sent = 0
    errors = []
    delay = get_delay()
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"📤 Начинаю обработку {total} объявлений..."
    )

    for i, ad in enumerate(current_ads, 1):
        try:
            token = random.choice(current_tokens)
            proxy = random.choice(current_proxies) if current_proxies else None
            text_template = random.choice(current_texts)

            ad_id = extract_ad_id_from_url(ad.get('url', ''))
            if not ad_id:
                errors.append(f"{i}. {ad.get('title', '')[:30]}: ID не найден в URL")
                continue
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"⚠️ {i}. {str(e)[:200]}"
            )

    # Сохраняем статистику отправки
...
async def handle_csv_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if parsed:
            ...
            save_ads(ads)
            await update.message.reply_text(
                f"✅ CSV загружен! Объявлений: <b>{len(ads)}</b>",
                parse_mode='HTML',
                reply_markup=main_menu()
            )
        else:
            await update.message.reply_text("❌ Не удалось распарсить CSV.\nПроверьте формат.", reply_markup=back_button())
    except Exception as e:
