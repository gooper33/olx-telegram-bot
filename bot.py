...
def extract_ad_id_from_url(url):
    """Извлекает ID объявления из URL (несколько способов)"""
    if not url:
        return None
    
    # Способ 1: формат ...-IDkU7Kk.html
    match = re.search(r'-([A-Za-z0-9]+)\.html$', url)
    if match:
        return match.group(1)
    
    # Способ 2: формат .../IDkU7Kk
    match = re.search(r'/([A-Za-z0-9]+)$', url)
    if match:
        return match.group(1)
    
    # Способ 3: формат ...?id=IDkU7Kk
    match = re.search(r'[?&]id=([A-Za-z0-9]+)', url)
    if match:
        return match.group(1)
    
    # Способ 4: просто ищем любой ID формата IDkU... в URL
    match = re.search(r'(IDk[A-Za-z0-9]+)', url)
    if match:
        return match.group(1)
...
def send_offer_via_api(ad_url, offer_price, original_price):
    ...
    try:
        ...
        if response.status_code == 404:
            return {"error": f"Объявление {ad_id} не найдено (возможно, снято с публикации)"}
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "help":
        await query.edit_message_text(
            ...
            "📌 Как это работает:\n"
            "Бот предлагает цену на 1 лей меньше!",
            parse_mode='HTML',
